...
WriteToDebug("strResult="+strResult);strID=CreateValidIdentifier(strID);
...
WriteToDebug("strResult="+strResult);blnResult=SCORM_CallLMSSetValue("cmi.interactions."+intInteractionIndex+".id",strID);blnResult=SCORM_CallLMSSetValue("cmi.interactions."+intInteractionIndex+".type",scormInteractionType)&&blnResult;
if(strResponse!==null){
...
if(!blnUnloading){return origResult;}
...
if(result&&result.err!==null){_TCAPI_HandleRequestErrorResult(result,"Failed to save state ("+key+")");return false;}
return true;}
...
return data;}
function TCAPI_SetDataChunk(value){WriteToDebug("In TCAPI_SetDataChunk");TCAPI_ClearErrorInfo();return _TCAPI_SetStateSafe(TCAPI_STATE_SUSPEND_DATA,value);}
...
if(tcapi_cache.statementQueue.length>0){requestResults=tincan.sendStatements(tcapi_cache.statementQueue);if(typeof requestResults.results!=="undefined"&&requestResults.results.length>0){result=requestResults.results[0];if(result.err!==null){_TCAPI_HandleRequestErrorResult(result,"Failed to commit data: statements");if(statementAdded){tcapi_cache.statementQueue.pop();TCAPI_UPDATES_PENDING=true;}
return false;}}
tcapi_cache.statementQueue=[];}
return true;}
function TCAPI_Finish(exitType,statusWasSet){WriteToDebug("In TCAPI_Finish - exitType: "+exitType);TCAPI_ClearErrorInfo();if(exitType===EXIT_TYPE_SUSPEND){_TCAPI_SetStateSafe(TCAPI_STATE_TOTAL_TIME,TCAPI_GetPreviouslyAccumulatedTime()+GetSessionAccumulatedTime());TCAPI_SetSuspended();}
...
function TCAPI_SetLanguagePreference(strLanguage){WriteToDebug("In TCAPI_SetLanguagePreference strLanguage="+strLanguage);TCAPI_ClearErrorInfo();return _TCAPI_SetStateSafe("cmi.student_preference.language",strLanguage);}
...
function TCAPI_SetTextPreference(intPreference){WriteToDebug("In TCAPI_SetTextPreference intPreference="+intPreference);TCAPI_ClearErrorInfo();return _TCAPI_SetStateSafe("cmi.student_preference.text",intPreference);}
...
result=cmi5.getLRS().saveStatements(CMI5_STATEMENT_QUEUE);if(result.err!==null){errMesg="Failed to commit data: statements";if(/^\d+$/.test(result.err)){if(result.err===0){errCode=CMI5_CONN_ERROR;errDiag="Aborted, offline, or invalid CORS endpoint";}
else{errCode=result.err;errDiag=result.xhr.responseText;}}
else{errCode=CMI5_DEP_ERROR;errDiag=result.err;}
CMI5_SetErrorInfoManually(errCode,errMesg,errDiag);return false;}
...
function CMI5_SetFailed(){WriteToDebug("In CMI5_SetFailed");var masteryScore,auActivityId,tempSt,i,st;CMI5_ClearErrorInfo();CMI5_ResetBlnStatusWasSetIfAppropriate();if(cmi5.getLaunchMode()!=="Normal"){CMI5_SetErrorInfoManually(CMI5_ERROR_NON_NORMAL_MODE,"Failed to SetFailed","Launch mode ("+cmi5.getLaunchMode()+") not a recording mode");return false;}
...
function CMI5_SetPassed(){WriteToDebug("In CMI5_SetPassed");var masteryScore,auActivityId,tempSt,i,st;CMI5_ClearErrorInfo();CMI5_ResetBlnStatusWasSetIfAppropriate();if(cmi5.getLaunchMode()!=="Normal"){CMI5_SetErrorInfoManually(CMI5_ERROR_NON_NORMAL_MODE,"Failed to SetPassed","Launch mode ("+cmi5.getLaunchMode()+") not a recording mode");return false;}
...
function CMI5_ResetBlnStatusWasSetIfAppropriate(){var isAlreadyComplete=CMI5_PENDING_STATUS.completion||CMI5_COMMITTED_STATUS.completion;if(!isAlreadyComplete){WriteToDebug("Overriding blnStatusWasSet to false.");blnStatusWasSet=false;}}
...
function Unload(){WriteToDebug("In Unload");ClearErrorInfo();blnUnloading=true;return ExecFinish(DEFAULT_EXIT_TYPE);}
